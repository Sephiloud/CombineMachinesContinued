...
namespace CombineMachines
{
    public class ModEntry : Mod
    {
...
#if DEBUG
        internal static LogLevel InfoLogLevel = LogLevel.Trace; //Debug
#else
        ...
        public override void Entry(IModHelper helper)
        {
            ...
            helper.Events.GameLoop.GameLaunched += (sender, e) => AddCompatibilityRegistrations();
        }

        private void AddCompatibilityRegistrations()
        {
            const string BetterGameMenuUniqueId = "leclair.bettergamemenu";
            if (Helper.ModRegistry.IsLoaded(BetterGameMenuUniqueId))
            {
                Logger.Log($"{nameof(ModEntry)}.{nameof(AddCompatibilityRegistrations)}: {BetterGameMenuUniqueId} found, adding Compatibility Patch!", InfoLogLevel);
                BetterGameMenuApi = Helper.ModRegistry.GetApi<IBetterGameMenuApi>(BetterGameMenuUniqueId);
                try
                {
                    BetterGameMenuApi = Helper.ModRegistry.GetApi<IBetterGameMenuApi>(BetterGameMenuUniqueId);
                    BetterGameMenuApi?.OnPageOverlayCreation(pageCreationEvent => {
                        if (pageCreationEvent.Tab == nameof(VanillaTabOrders.Inventory) && pageCreationEvent.Page is InventoryPage inventory)
                            pageCreationEvent.AddOverlay(new InventoryMenuDrawOverlay(inventory));
                    });
                } catch (Exception ex)
                {
                    Logger.Log($"{nameof(ModEntry)}.{nameof(AddCompatibilityRegistrations)}: Could not get API for mod BetterGameMenuApi due to error: {ex}", LogLevel.Warn);
                    BetterGameMenuApi = null;
                }
            }
        }
        ...
        private void TryRegisterGMCM()
        {
            ...
            API.AddNumberOption(
                ModManifest,
                () => UserConfig.ToolTipOffset.X,
                (val) => UserConfig.ToolTipOffset = new(val, UserConfig.ToolTipOffset.Y),
                () => "ToolTips: X Offset",
                () => "Set a X-Offset in pixel for the Combined Machine ToolTip.\n\nRecommended value: 0, except for Compatibility with other Mods like UI Info Suite 2 (Alternative)."
            );

            API.AddNumberOption(
                ModManifest,
                () => UserConfig.ToolTipOffset.Y,
                (val) => UserConfig.ToolTipOffset = new(UserConfig.ToolTipOffset.X, val),
                () => "ToolTips: Y Offset",
                () => "Set a Y-Offset in pixel for the Combined Machine ToolTip.\n\nRecommended value: 0, except for Compatibility with other Mods like UI Info Suite 2 (Alternative)."
            );
        }
        ...
        private void Input_ButtonPressed(object sender, ButtonPressedEventArgs e)
        {
            ...
            {
                if (TryGetClickedInventoryItem(e, out Item ClickedItem, out int ClickedItemIndex))
                {
        ...
        private static bool TryGetClickedInventoryItem(ButtonPressedEventArgs e, out Item Result, out int Index)
        {
            Result = null;
            Index = -1;

            InventoryPage InvPage = null;
            IClickableMenu PossiblyGameMenu = Game1.activeClickableMenu;
            if (PossiblyGameMenu is GameMenu GM && GM.currentTab == GameMenu.inventoryTab)
            {
                InvPage = GM.pages.First(x => x is InventoryPage) as InventoryPage;
            }
            else if (BetterGameMenuApi != null && BetterGameMenuApi.IsMenu(PossiblyGameMenu) && BetterGameMenuApi.AsMenu(PossiblyGameMenu)?.CurrentTab == "Inventory")
            {
                InvPage = BetterGameMenuApi.GetCurrentPage(PossiblyGameMenu) as InventoryPage;
            }
            else return false;
            if (InvPage == null) return false;
